...
<head>
    <title>留言咨询查看列表</title>
    <%@include file="common/head.jsp" %>
    <%@include file="common/loginstate.jsp" %>
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/liuyanChakanMy?d=1";
        ...
        function liuyanDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/liuyanDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/liuyanChankanMy";
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/liuyanChakanMy" name="formSearch" id="formSearch" method="post">
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">留言咨询查看</td>
    </tr>
    ...
    <c:forEach items="${pageInfo.records}" var="info" varStatus="s">
        <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';"
            ...
            <td bgcolor="#FFFFFF" align="center">
                <c:choose>
                    <c:when test="${fn:length(info.tianjiashijian)>'16'}">
                        ${fn:substring(info.tianjiashijian,0,16)}...
                    </c:when>
                    <c:otherwise>
                        ${info.tianjiashijian}
                    </c:otherwise>
