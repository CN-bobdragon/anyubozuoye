...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/huodongChakanMy?d=1";
            window.location.href=url;
        }
        //删除信息
        function huodongDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/huodongDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/huodongChankanMy";
        ...
        //跳转到信息修改页
        function huodongGet(id) {
            window.location.href = "${contextPath}/huodongGet/" + id;
        }

        //跳转到信息详细页
        function huodongDetail(id) {
            window.location.href = "${contextPath}/huodongDetail/" + id;
        }
        ...
        //更新信息
        function huodongSet(id,property,value) {
            if (confirm('您确定操作吗?')) {
                var param = {};
                param["id"]=id;
                param[property]=value;
                $.post(basePath + '/huodongSet', param, function(resp) {
                    if (resp.success) {
                        alert("操作成功！");
                        //window.location.href = "${contextPath}/huodongChankanMy";
                        document.formSearch.submit();
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/huodongChakanMy" name="formSearch" id="formSearch" method="post">
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">活动查看</td>
    </tr>
...
<%--                <input type="button" value="修改" onclick="huodongGet(${info.id})"/>--%>
                <input type="button" value="详情" onclick="huodongDetail(${info.id})"/>
<%--                <input type="button" value="删除" onclick="huodongDel(${info.id})"/>--%>
                    <%--sonTable本表子表管理--%>
                <%--<input type="button" value="活动报名查看" onclick="huodongbaomingGetByhuodongid(${info.id})"/>--%>
                <%--<input type="button" value="活动报名添加" onclick="huodongbaomingAdd(${info.id})"/>--%>
                <%--<input type="button" value="活动小组查看" onclick="huodongxiaozuGetByhuodongid(${info.id})"/>--%>
                <%--<input type="button" value="活动小组添加" onclick="huodongxiaozuAdd(${info.id})"/>--%>
