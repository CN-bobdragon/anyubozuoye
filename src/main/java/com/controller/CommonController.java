...
public class CommonController extends BaseController {
    ...
    @ResponseBody
    public ApiResponse<String> uploadfileNew(@RequestParam("file") MultipartFile file, HttpServletRequest request) throws Exception {
        if (file.isEmpty()) {
            return ApiResponse.failed("文件不能为空");
        }
        String fujianFileName = file.getOriginalFilename();
        String newFujianName = new Date().getTime() + fujianFileName.substring(fujianFileName.indexOf("."));
        String dstPath = request.getSession().getServletContext().getRealPath("upload") + "\\" + newFujianName;
        File dstFile = new File(dstPath);
        if (!dstFile.getParentFile().exists()) {
        ...
        return ApiResponse.success("/upload/" + newFujianName);
    }
