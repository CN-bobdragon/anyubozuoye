...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/shenqingChakanMy?d=1";
        ...
        function shenqingDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/shenqingDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/shenqingChankanMy";
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/shenqingChakanMy" name="formSearch" id="formSearch" method="post">
        小组:
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">申请查看</td>
    </tr>
...
<%--
            <c:choose>
                ...
                <input type="button" value="审核" onclick="shenqingSet(${info.id},'shenhe','yes')"/>
                </c:when>
                <c:otherwise>
                <input type="button" value="取消" onclick="shenqingSet(${info.id},'shenhe','no')"/>
                </c:otherwise>
            </c:choose>
 --%>
