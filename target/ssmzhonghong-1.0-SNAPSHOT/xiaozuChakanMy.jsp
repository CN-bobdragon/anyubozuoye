...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/xiaozuChakanMy?d=1";
            window.location.href=url;
        }
        //删除信息
        function xiaozuDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/xiaozuDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/xiaozuChankanMy";
        ...
        //跳转到信息修改页
        function xiaozuGet(id) {
            window.location.href = "${contextPath}/xiaozuGet/" + id;
        }

        //跳转到信息详细页
        function xiaozuDetail(id) {
            window.location.href = "${contextPath}/xiaozuDetail/" + id;
        }
        ...
        //更新信息
        function xiaozuSet(id,property,value) {
            if (confirm('您确定操作吗?')) {
                var param = {};
                param["id"]=id;
                param[property]=value;
                $.post(basePath + '/xiaozuSet', param, function(resp) {
                    if (resp.success) {
                        alert("操作成功！");
                        //window.location.href = "${contextPath}/xiaozuChankanMy";
                        document.formSearch.submit();
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/xiaozuChakanMy" name="formSearch" id="formSearch" method="post">
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">小组查看</td>
    </tr>
...
<%--                <input type="button" value="修改" onclick="xiaozuGet(${info.id})"/>--%>
                <input type="button" value="详情" onclick="xiaozuDetail(${info.id})"/>
<%--                <input type="button" value="删除" onclick="xiaozuDel(${info.id})"/>--%>
                    <%--sonTable本表子表管理--%>
                <%--<input type="button" value="小组申请查看" onclick="xiaozushenqingGetByxiaozuid(${info.id})"/>--%>
                <%--<input type="button" value="小组申请添加" onclick="xiaozushenqingAdd(${info.id})"/>--%>
